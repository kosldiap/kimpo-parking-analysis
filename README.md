...
### ① 국내선 만성 초과수요 — 신축의 정량 근거
- 최대 점유 **122%**(주차대행 overflow), **만차 연 1,018h(≈42일)**, 설계기준(연 30위) 점유 **115%**, 만차일수(공식) 국내1P 연 **365일**, 입차 탐색시간 2014년 2:40→2015년 5:26.
- 국제선·화물·직원은 만차 0h(여유) → **국내선만 구조적 부족**. 신축 규모 ≈ 국내선 정원 +15%.

![부하지속곡선](reports/figures/17_load_duration.png)

### ② 유형별 수요·운영 특성이 완전히 다름
- **시간대 동인**: 국내선 새벽5시(여객) / 화물 업무시간 쌍봉(물류) → 통합이 아닌 **유형별 개별 모형**.
- **회전율**: 화물 1.15회/면/일(회전형) ≫ 국내선 0.57(점유형) → 점유형(국내·국제)이 카셰어링/AV에 취약.
- **명절·연휴 배율**: 여객 0.85~1.0(거의 무관) vs 화물·직원 0.23~0.33(급감) → 여객 상시수요 / 업무일 패턴.
- **게이트 부하**: 국내선 입차피크 5시지만 입+출 **합피크는 18시** → 게이트·차로 설계는 합피크 기준.

![시간대 프로파일](reports/figures/03_hourly_profile.png)

### ③ 여객↔주차 관계 (장래예측 기반)
- 공식자료 대비 재구성 **98% 일치** → 여객 기반 장래수요 추정의 신뢰 기반.
- **주차가 항공 출발을 2시간 선행**(교차상관) — 수속·보안·대기·쇼핑 리드타임.

### ④ 자율주행/카셰어링 취약성
- 공항 여객주차 점유의 **96%(국내선)/94%(국제선)가 다일 여행주차** = 카셰어링 드롭오프로 제거가능. 화물 42%·직원 28%는 견고. → 주차빌딩 좌초자산 위험.

### ⑤ 매출·감면 구조
- 국내선 요금 623억/수입 529억, **감면율 15.1%**(화물 21%·직원 5%). 건당수입 국내선 20,949원(장기) vs 국제선 10,606원.
